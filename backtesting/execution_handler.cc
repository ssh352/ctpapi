...
SimulatedExecutionHandler::SimulatedExecutionHandler(MailBox* mail_box)
    : mail_box_(mail_box) {
...
void SimulatedExecutionHandler::HandleTick(
  ...
  if (!long_limit_orders_.empty()) {
    ...
    std::for_each(
        long_limit_orders_.begin(), end_it, [=](const LimitOrder& lo) {
          EnqueueRtnOrderEvent(lo.instrument, lo.order_id, lo.direction,
...
void SimulatedExecutionHandler::HandlerInputOrder(
...
void SimulatedExecutionHandler::HandleCancelOrder(
  ...
  find_it = std::find_if(short_limit_orders_.begin(), short_limit_orders_.end(),
                         [=](const LimitOrder& limit_order) {
                           return limit_order.order_id == order_id;
                         });
...
void SimulatedExecutionHandler::EnqueueRtnOrderEvent(
...
void SimulatedExecutionHandler::EnqueueCancelOrderEvent(
    const LimitOrder& limit_order) {
  auto order = std::make_shared<OrderField>();
...
double SimulatedExecutionHandler::GetFillPrice() const {
  return current_tick_->last_price;
}
