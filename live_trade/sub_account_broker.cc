...
void CAFSubAccountBroker::ReturnOrderField(const std::shared_ptr<OrderField>& order)
{
  inner_mail_box_->Send(order);
}

void CAFSubAccountBroker::HandleCancelOrder(const CTPCancelOrder& cancel_order)
{
...
void CAFSubAccountBroker::MakeCtpInstrumentBrokerIfNeed(const std::string& instrument)
{
...
CAFSubAccountBroker::CAFSubAccountBroker(caf::actor_config& cfg, LiveTradeMailBox* inner_mail_box, LiveTradeMailBox* common_mail_box, std::string account_id) : caf::event_based_actor(cfg),
