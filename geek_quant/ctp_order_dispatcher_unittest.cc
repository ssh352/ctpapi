...
TEST(CtpOrderDispatcherTest, OpenOrder) {
  CtpOrderDispatcher dispatcher(true);
  {
    ...
    EXPECT_EQ(OldOrderStatus::kOpening, order->order_status);
...

  {
    ...
    EXPECT_EQ(OldOrderStatus::kOpened, order->order_status);
  }
...
TEST(CtpOrderDispatcherTest, CloseOrder) {
  CtpOrderDispatcher dispatcher(true);
  {
    ...
    EXPECT_EQ("abc", order->instrument);
    EXPECT_EQ("00001", order->order_no);
    EXPECT_EQ(OldOrderStatus::kCloseing, order->order_status);
    EXPECT_EQ(1234.1, order->order_price);
    EXPECT_EQ(10, order->volume);
  }
...

  {
    ...
    EXPECT_TRUE(order);
    EXPECT_EQ(1, order->volume);
    EXPECT_EQ(OldOrderStatus::kClosed, order->order_status);
  }
...
TEST(CtpOrderDispatcherTest, CancelOpenOrder) {
...

  {
    ...
    EXPECT_TRUE(order);
    EXPECT_EQ("00001", order->order_no);
    EXPECT_EQ(OldOrderStatus::kOpenCanceled, order->order_status);
  }
}
...
TEST(CtpOrderDispatcherTest, CancelCloseOrder) {
...

  {
    ...
    EXPECT_TRUE(order);
    EXPECT_EQ("00001", order->order_no);
    EXPECT_EQ(OldOrderStatus::kCloseCanceled, order->order_status);
  }
}
