#include "follow_strategy/unittest/follow_strategy_servcie_fixture.h"
#include "gtest/gtest.h"

// Test Open
TEST_F(FollowStragetyServiceFixture, OpenBuy) {
  auto ret = PushNewOpenOrderForMaster("1");

  OrderInsertForTest order_insert = std::get<0>(ret);

  EXPECT_EQ("1", order_insert.order_id);
...
TEST_F(FollowStragetyServiceFixture, CloseOrderCase3) {
...

  {
    ...
    auto order_insert = std::get<0>(ret);
    EXPECT_EQ("", order_insert.order_id);
...
TEST_F(FollowStragetyServiceFixture, CloseOrderCase5) {
  ...
  std::string slave_order_id;
  {
    auto ret = PushNewCloseOrderForMaster("4", OrderDirection::kSell, 4);

    auto order_insert = std::get<0>(ret);
    slave_order_id = order_insert.order_id;
    EXPECT_EQ("4", order_insert.order_id);
...

  {
    strategy_dispatch_.RtnOrder(
        MakeSlaveOrderData(slave_order_id, OrderDirection::kSell,
                           PositionEffect::kClose, OrderStatus::kActive, 0, 4));
  }

  {
    ...
    auto order_insert = std::get<0>(ret);
    EXPECT_EQ("5", order_insert.order_id);
    EXPECT_EQ(OrderDirection::kSell, order_insert.direction);
...
TEST_F(FollowStragetyServiceFixture, CancelOrderCase3) {
  ...
  signal_dispatch_->RtnOrder(
      MakeMasterOrderData("1", OrderDirection::kBuy, PositionEffect::kOpen,
...
TEST_F(FollowStragetyServiceFixture, CancelMultiOrder) {
...

  {
    ...
    EXPECT_EQ("", std::get<0>(ret).order_id);
